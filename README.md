...
## Worker v1.18.10 IDB解析詳細コピー版（力業補正なし）

v1.18.9のような特定IDハードコードではなく、SGKウェポンの実際の `data-page` 構造を確認するためのデバッグ版です。

変更:
- SGKウェポンなど特定IDへの既知補正は入れていません。
- `解析詳細をコピー` ボタンを追加。
- URL取得後に押すと、`props` / `weapon` / 必要スキルらしいパス / 実際の解析結果をJSONでコピーできます。
- `Worker接続テスト` ボタンはありません。

SGKウェポンで足りないスキルがある場合:
1. SGKウェポンURLを取得
2. `解析詳細をコピー`
3. コピーされたJSONを共有
4. その実データ構造に合わせて汎用パーサーを直す
