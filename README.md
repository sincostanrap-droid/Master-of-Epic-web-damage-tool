## v1.23.23 / 装備カタログ条件表示・ページ送り・追加効果二重加算修正

- 装備カタログαに「条件/変動」と「AC」列を追加し、必要スキル、現在スキルとの差、性能変動メモを確認できるようにしました。
- 装備Buffフィルタを「すべて / あり / なし」の選択式に変更し、Buffなし装備だけでも絞り込めるようにしました。
- カタログ結果にページ送りを追加しました。1ページあたり50/100/200/500/1000件を選び、2ページ目以降へ移動できます。
- カタログから装備登録へ追加するとき、公式DB由来の `addStatuses` と正規化済み `extraStats` を両方適用して追加効果が2倍になる問題を修正しました。`extraStats` は `addStatuses` がない場合のフォールバックとしてだけ使います。

## v1.23.22 / 装備Buff修正UI改善・カタログソート再投入

- 装備Buff修正タブをさらに整理し、同一 `technic_id` の既定グループを「手動競合グループ」として表示しないようにしました。内部では同一 `technic_id` は最新1つだけ有効です。
- 装備Buff修正タブの競合グループ欄は、ユーザーが手動で入力した競合グループだけを表示・管理します。
- 装備Buff修正タブに検索、手動競合ありのみ、効果入力済みのみの絞り込みを追加しました。
- 装備Buff修正カードの詳細追加ステータス/効果追加を折りたたみにし、主要数値と原文確認を優先するUIへ寄せました。
- 装備登録側の装備Buff編集欄を折りたたみ、通常は「装備Buff修正」タブで修正する前提にしました。
- 装備カタログαに、ソート、昇順/降順、表示件数の指定を再投入しました。

## v1.23.21 / 装備Buff修正UI整理・technic_id集約

- 「装備Buff修正」タブをBuff編集専用UIへ整理しました。装備本体の追加ステータス、武器計算、装備本体側の効果追加は表示しません。
- 同一 `technic_id` の装備Buffは1枚のカードに集約して表示します。編集内容は同じ `technic_id` を持つ装備行すべてへ反映されます。
- 装備Buff修正タブ上で `競合グループ` と `重複ルール` を直接設定できるようにしました。既定は `same-technic` で、同一technic_idは最新1つのみ有効です。
- Wiki原文 / Scrapbox根拠行 / 抽出ヒントは、集約カード内の説明欄として表示・編集できます。
- UIをカード形式に整理し、対象装備、効果要約、数値欄、原文欄を分けて見やすくしました。
...
- 同一 `technic_id` の装備Buffは内部処理で最新1つだけ有効にします。`technic-ID` は手動競合グループとしては表示しません。
...
現在の表示バージョン: **v1.23.23 / 装備カタログ条件表示・ページ送り・追加効果二重加算修正**
