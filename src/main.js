...
const APP_VERSION_NOTE = "戦闘ログ解析を追加・タブ初回表示まで遅延初期化";
...
function applyQuickEffectToRow(row, context, key, value, name="", scope="auto") {
  ...
  if (context === "composite") {
    const map = {
      ...
      convMagicRate:"convMagicRate", convMagicSpeedRate:"convMagicSpeedRate", convSpeedRate:"convSpeedRate",
      dmgPct:"dmgPct"
    };
  ...
  if (bodyScope && key === "attackFlat") row.attack = +(row.attack || 0) + v;
    ...
    const map = {
      ...
      convMagicRate:"equipBuffConvMagicRate", convMagicSpeedRate:"equipBuffConvMagicSpeedRate", convSpeedRate:"equipBuffConvSpeedRate",
...
function defaultEquipmentCandidate(slot, enabled=true) {
  return {
    ...
    equipBuffSourceText: "",
    equipBuffWikiText: "",
    equipBuffScrapboxText: "",
    equipBuffRuleConfidence: "",
    equipBuffRuleSource: "",
...
function normalizeEquipmentCandidate(row, fallbackSlot) {
  ...
  const out = {
    ...
    equipBuffScrapboxText: row?.equipBuffScrapboxText || "",
    equipBuffRuleConfidence: row?.equipBuffRuleConfidence || "",
    equipBuffRuleSource: row?.equipBuffRuleSource || "",
    ...
    equipBuffConvMagicSpeedRate: +(row?.equipBuffConvMagicSpeedRate || 0),
...
function normalizeEquipmentRows(rows) {
  ...
  // 両手武器が使用中なら反対側の手は使用不可。弾丸スロットは対象外。
  enforceWeaponHandExclusivity(result);
...
const DEFAULT_STATE = () => ({
  ...
  attackDps: defaultAttackDpsState(),
...
function equipmentBuffToCompositeRow(r) {
  ...
  const row = {
    ...
    convMagicSpeedRate: +r.equipBuffConvMagicSpeedRate || 0,
...
function resolveCompositeRowsForGroups(rows) {
  ...
  normalizeCompositeRows(rows).forEach((row, order) => {
    ...
    const candidate = {...row, _groupOrder: order, _groupScore: buffGroupResolveScore(row, order, "composite")};
    if (!byGroup.has(key)) {
...
function collectActiveBuffConflictCandidates(st) {
  ...
  normalizeCompositeRows(st.composite || []).forEach((row, idx) => {
    ...
    candidates.push({
      ...
      _groupOrder: order++,
      _groupScore: buffGroupResolveScore(row, order, "composite")
    });
  });
  ...
  (st.post || []).forEach((row, idx) => {
    ...
    candidates.push({
      ...
      _groupOrder: order++,
      _groupScore: buffGroupResolveScore(row, order, "post")
    });
  });
  ...
  (st.other || []).forEach((row, idx) => {
    ...
    candidates.push({
      ...
      _groupOrder: order++,
      _groupScore: buffGroupResolveScore(row, order, "other")
    });
  });
...
function weaponHandSlot(row) {
  const slot = String(row?.slot || row || "");
  return slot === "武器: 右手" || slot === "武器: 左手" ? slot : "";
}

function oppositeWeaponHandSlot(slot) {
  return slot === "武器: 右手" ? "武器: 左手"
    : slot === "武器: 左手" ? "武器: 右手"
    : "";
}

function weaponUsesBothHands(row) {
  return !!weaponHandSlot(row) && row?.weaponTwoHanded === "○";
}

function enforceWeaponHandExclusivity(rows, preferredRow=null) {
  const list = Array.isArray(rows) ? rows : [];
  const preferredSlot = preferredRow?.enabled !== false ? weaponHandSlot(preferredRow) : "";

  if (preferredSlot) {
    const oppositeSlot = oppositeWeaponHandSlot(preferredSlot);
    if (weaponUsesBothHands(preferredRow)) {
      list.forEach(row => {
        if (row !== preferredRow && row.slot === oppositeSlot) row.enabled = false;
      });
    } else {
      list.forEach(row => {
        if (row !== preferredRow && row.slot === oppositeSlot && weaponUsesBothHands(row)) row.enabled = false;
      });
    }
    return list;
  }

  const activeTwoHanded = list.find(row => row.enabled !== false && weaponUsesBothHands(row));
  if (activeTwoHanded) {
    const oppositeSlot = oppositeWeaponHandSlot(activeTwoHanded.slot);
    list.forEach(row => {
      if (row !== activeTwoHanded && row.slot === oppositeSlot) row.enabled = false;
    });
  }
  return list;
}
...
function syncSelectedWeaponToHiddenInputs() {
  ...
  if (!w) {
    setHiddenValue("weaponDamage", 0);
    setHiddenValue("weaponWeight", 0);
    setHiddenValue("weaponRange", 0);
    setHiddenValue("weaponSkill", 0);
    setHiddenValue("requiredSkill", 0);
    return;
  }
...
function makeEquipmentBuffSourcePanel(row, preview=equipmentBuffResolvedPreview(row)) {
  ...
  const makeBlock = (title, text, prop) => {
    const label = document.createElement("label");
    label.className = "equipBuffSourceLabel";
    label.textContent = title;
    const area = makeCell("textarea", {rows:"4"});
    area.className = "equipBuffSourceText";
    area.value = text || "";
    area.placeholder = `${title} は空です`;
    area.oninput = () => { row[prop] = area.value; };
...
function makeEquipmentBuffEditor(row, statusButton) {
  ...
  buffSummary.textContent = "装備Buffの詳細を編集";
  ...
  buffGrid.appendChild(equipBuffNumberInput(row, "equipBuffConvMagicSpeedRate", "魔力→移動速度%", "1"));
...
function equipmentUseCell(row, idx) {
  ...
  cb.onchange = () => {
    ...
    if (cb.checked) {
      ...
      target.enabled = true;
      // 今ONにした武器を優先し、両手武器と反対手武器の同時使用を防ぐ。
      enforceWeaponHandExclusivity(state.equipment, target);
    } else {
...
/* Showcase の描画・テキスト・コピー処理は src/ui/showcase.js へ分離しました。 */
...
/* IDB装備インポートのプレビュー・候補追加処理は src/ui/idbImportPreview.js へ分離しました。 */

/* カタログデータの遅延読込・重複排除処理は src/domain/catalogData.js へ分離しました。 */

function equipBuffRuleKeyCandidates(buff, item=null) {
  const keys = [];
  const add = v => { const s = String(v || "").trim(); if (s && !keys.includes(s)) keys.push(s); };
  add(buff?.catalogId); add(buff?.id); add(buff?.officialTechnicId); add(item?.technicId);
  if (buff?.officialTechnicId) add(`technic-${buff.officialTechnicId}`);
  if (item?.technicId) add(`technic-${item.technicId}`);
  add(buff?.name); add(item?.equipBuff?.name);
  return keys;
}
...
function setIfNumeric(row, prop, value, overwrite=false) {
  const v = +value || 0;
  if (!v) return false;
  if (!overwrite && +(row[prop] || 0)) return false;
  row[prop] = v;
  return true;
}
...
function applyEquipBuffRuleCandidateToEquipment(row, rule, opts={}) {
  ...
  const statMap = {
    ...
    dmgPct:"equipBuffDmgPct",
    extraAC:"equipBuffExtraAC", extraACPct:"equipBuffExtraACPct", extraHP:"equipBuffExtraHP", extraHPPct:"equipBuffExtraHPPct", extraMP:"equipBuffExtraMP", extraMPPct:"equipBuffExtraMPPct", extraST:"equipBuffExtraST", extraSTPct:"equipBuffExtraSTPct",
    extraMaxWeight:"equipBuffExtraMaxWeight", extraMaxWeightPct:"equipBuffExtraMaxWeightPct", extraHit:"equipBuffExtraHit", extraHitPct:"equipBuffExtraHitPct", extraAvoid:"equipBuffExtraAvoid", extraAvoidPct:"equipBuffExtraAvoidPct",
    ...
    extraCarpentryGradeZone:"equipBuffExtraCarpentryGradeZone", extraCarpentryGaugeSlip:"equipBuffExtraCarpentryGaugeSlip", extraCarpentryHitZone:"equipBuffExtraCarpentryHitZone",
    extraTailoringGradeZone:"equipBuffExtraTailoringGradeZone", extraTailoringGaugeSlip:"equipBuffExtraTailoringGaugeSlip", extraTailoringHitZone:"equipBuffExtraTailoringHitZone",
    extraDecorationGradeZone:"equipBuffExtraDecorationGradeZone", extraDecorationGaugeSlip:"equipBuffExtraDecorationGaugeSlip", extraDecorationHitZone:"equipBuffExtraDecorationHitZone",
    extraCookingGradeZone:"equipBuffExtraCookingGradeZone", extraCookingGaugeSlip:"equipBuffExtraCookingGaugeSlip", extraCookingHitZone:"equipBuffExtraCookingHitZone",
    extraBrewingGradeZone:"equipBuffExtraBrewingGradeZone", extraBrewingGaugeSlip:"equipBuffExtraBrewingGaugeSlip", extraBrewingHitZone:"equipBuffExtraBrewingHitZone",
    extraAlchemyGradeZone:"equipBuffExtraAlchemyGradeZone", extraAlchemyGaugeSlip:"equipBuffExtraAlchemyGaugeSlip", extraAlchemyHitZone:"equipBuffExtraAlchemyHitZone",
  ...
  if (setIfNumeric(row, "equipBuffConvMagicSpeedRate", conv.magicToSpeedPct, overwrite)) applied = true;
  if (setIfNumeric(row, "equipBuffConvSpeedRate", conv.speedToAttackPct, overwrite)) applied = true;

  (rule.skillEffects || []).forEach(e => {
    const name = e.name || e.target || "";
    const value = +e.value || 0;
    if (name || value) { pushDisplayEffect(row, "skillPlus", value, name, "", "display", "TSV候補"); applied = true; }
  });
  (rule.customEffects || []).forEach(e => {
    const name = e.name || e.target || "";
    const value = +e.value || 0;
  ...
  row.equipBuffSourceText = appendUniqueText(row.equipBuffSourceText, rule.parsedStatsHint || rule.memo || "");
  row.equipBuffWikiText = appendUniqueText(row.equipBuffWikiText, rule.rawInfo || "");
  row.equipBuffScrapboxText = appendUniqueText(row.equipBuffScrapboxText, rule.scrapboxRawLines || "");
  return applied;
}


/* カタログの検索・数値フィルター処理は src/domain/catalogSearch.js へ分離しました。 */

/* カタログ結果のページ管理・装備登録処理は src/ui/catalogResults.js へ分離しました。 */

/* カタログの必要スキル・性能要約・並び替え処理は src/domain/catalogSort.js へ分離しました。 */

/* カタログのフィルターUI・初回タブ表示処理は src/ui/catalogTab.js へ分離しました。 */

/* カタログ結果行の表示処理は src/ui/catalogRows.js へ分離しました。 */


/* 装備Buff修正タブは廃止。装備Buffの編集は装備登録タブ内の詳細欄に統合する。 */
...
function bindCalcRefreshInput(el) {
  // Catalog controls only narrow/sort the catalog. They must not trigger the
  // simulator-wide calculation, especially when the restored initial tab is
  // Catalog and these controls already exist during application startup.
  if (!el || el.closest?.('[data-tab-panel="catalog"]')) return;
  if (el.dataset.moeCalcRefreshBound === "1") return;
...
/* 構成診断・プリセット比較処理は src/ui/buildAnalysis.js へ分離しました。 */
  ...
  function installCatalogInputDebounceHints() {
    ...
    document.querySelectorAll("#catalogSearch, #catalogCategory, #catalogSlot, [id^='catalogStat'], [id^='catalogBuffEffect'], #catalogBuffMode, #catalogSort, #catalogSortDir, #catalogLimit").forEach(el => {
      ...
      el.addEventListener("input", () => {
        if (typeof global.applyCatalogSearch === "function") return;
        if (global.renderCatalogResults) global.renderCatalogResults();
      }, {passive: true});
...
/* Showcase のスキル強化合計処理は src/ui/showcaseSkillPlusTotals.js へ分離しました。 */

/* 装備カタログの検索適用・結果キャッシュ処理は src/ui/equipmentCatalogSearchCache.js へ分離しました。 */
