...
// __MOE_EQUIPMENT_BUFF_SPECIALIZED_EFFECT_REVIEW_BULK22_V1__
// リポジトリ収録済みのWiki原文とScrapbox照合結果を再比較し、
// 現行計算モデル外の技能別補正・条件付き補正・追撃を表示用へ分類する。
(() => {
  const display = (id, effects, memo, skillEffects = []) => {
    const rule = window.MOE_BUFF_RULES_MANUAL[`technic-${id}`];
    rule.verified = true;
    rule.reviewStatus = "display-only";
    rule.customEffects = (Array.isArray(effects) ? effects : [effects])
      .map(name => ({ name, value: 0 }));
    rule.skillEffects = skillEffects;
    rule.memo = memo;
  };

  display(
    12307,
    ["筋力スキル効果+20相当", "専用技「ノア ガトリング」使用可能"],
    "Scrapboxのライオニック オーラ効果時間から筋力スキル+20相当を確認。現行物理ダメージ計算へは直接加算しない。",
    [{ name: "筋力", value: 20 }]
  );
  display(7624, "牙攻撃力上昇（上昇量不明）", "Wikiで効果を確認。牙専用補正の計算式・上昇量が不明なため表示のみ。");
  display(
    7093,
    "キック攻撃力補正+10",
    "Scrapboxのキック攻撃力増加バフ一覧で+10を確認。キック専用補正のため表示のみ。",
    [{ name: "キック", value: 10 }]
  );
  display(
    7917,
    ["日中のみ攻撃力+3%", "日中のみ回避+3%"],
    "Scrapboxの時間限定バフ記録を反映。時間条件を現行計算モデルで扱わないため表示のみ。"
  );
  display(
    12334,
    ["銃器テクニックのディレイ短縮（短縮量不明）", "専用技「タンク キャノン」使用可能"],
    "Wikiで効果を確認。技能別ディレイの数値が不明なため表示のみ。"
  );
  display(
    9603,
    ["投げディレイ約-15%", "一部投げ技の効果：投げスキル+20相当"],
    "Scrapboxの検証記録（-15%程度、スキル+20程度）を概算表記のまま反映。技能別補正のため表示のみ。",
    [{ name: "投げ", value: 20 }]
  );
  display(8253, ["牙攻撃力上昇（上昇量不明）", "待機・移動モーション変化"], "Wikiで効果を確認。牙専用補正の上昇量が不明なため表示のみ。");
  display(6604, ["牙攻撃力上昇（上昇量不明）", "牙命中上昇（上昇量不明）"], "Wikiで効果を確認。牙専用補正の数値が不明なため表示のみ。");
  display(12704, ["牙攻撃力上昇（上昇量不明）", "牙命中上昇（上昇量不明）"], "Wikiで効果を確認。牙専用補正の数値が不明なため表示のみ。");
  display(
    7824,
    ["シャウト詠唱速度約-35%（モーション約-20%）", "シャウトディレイ-20%", "シャウト効果範囲+2m"],
    "Scrapboxの複数装備記録でディレイ-20%・範囲+2mを確認。詠唱速度は推定値として表示のみ。"
  );
  display(
    13877,
    ["水泳スキル依存効果上昇（上昇量不明）", "通常攻撃命中時に直線範囲追撃「海流撃」"],
    "WikiとScrapboxで発動条件・直線範囲追撃を確認。水泳補正量と追撃威力が不明で現行計算モデル外のため表示のみ。"
  );
  display(6368, ["待機・移動モーション変化", "牙攻撃力・牙命中が大幅上昇（上昇量不明）"], "Wikiで効果を確認。牙専用補正の数値が不明なため表示のみ。");
  display(7873, ["牙命中上昇（上昇量不明）", "専用技「チューチュー ドレイン」使用可能"], "Wikiで効果を確認。牙専用補正の上昇量が不明なため表示のみ。");
})();
...
// New Buffs reviewed against uploaded Wiki on 2026-09-22.
Object.assign(window.MOE_BUFF_RULES_MANUAL, {
  "technic-14635": {
    "name": "神楽",
    ...
    "memo": "2026-09-22添付Wiki照合。暫定グループの？はユーザー指定により正規化。スキル強化は表示・検索用。魔法・属性・範囲・条件付き効果は表示用。WarAge無効。",
    "authoritativeStats": []
  },
  "technic-14634": {
    "name": "あなたの後ろにいるの",
  ...
  "technic-14633": {
    ...
    "stackRule": "same-technic",
    "stats": {
      "hpRegenPerMinute": 56.25,
      "mpRegenPerMinute": 56.25,
      "stRegenPerMinute": 56.25
    },
    "skillEffects": [],
    "customEffects": [
      {
  ...
  "technic-14618": {
    ...
    "customEffects": [
      {
        "name": "牙の直接攻撃のみ威力・命中がスキル+20相当（牙Buffの時間・効果量は不変）",
        "value": 0,
        "unit": ""
      },
      {
        "name": "与ダメージの10%をHP回復（与ダメ回復は資源種別を問わず最古の1件）",
        "value": 0,
        "unit": ""
      },
      {
  ...
  "technic-14611": {
    ...
    "memo": "2026-09-22添付Wiki照合。暫定グループの？はユーザー指定により正規化。スキル強化は表示・検索用。魔法・属性・範囲・条件付き効果は表示用。WarAge無効。 一部効果量・発動条件は未検証。",
    "authoritativeStats": []
  },
  "technic-14610": {
    "name": "スペル チャージ",
  ...
  "technic-14580": {
    ...
    "memo": "2026-09-22添付Wiki照合。暫定グループの？はユーザー指定により正規化。スキル強化は表示・検索用。魔法・属性・範囲・条件付き効果は表示用。WarAge無効。 物理与ダメージ約10%を計算へ適用。",
    "authoritativeStats": [
      "dmgPct",
      "extraMaxWeight"
    ]
  ...
  "technic-14577": {
    ...
    "customEffects": [
      {
        "name": "ドラゴン特攻・HP自然回復増加（倍率・回復量未確認）",
        "value": 0,
        "unit": ""
      },
      {
    ...
    "memo": "2026-09-22添付Wiki照合。暫定グループの？はユーザー指定により正規化。スキル強化は表示・検索用。魔法・属性・範囲・条件付き効果は表示用。WarAge無効。 一部効果量・発動条件は未検証。 物理与ダメージ約10%を計算へ適用。",
    "authoritativeStats": [
      "dmgPct"
    ]
  },
  ...
  "technic-14540": {
    ...
    "customEffects": [
      {
        "name": "不可視を見る・被ダメージ時に確率でミニ バエル召喚（発動率未確認）",
        "value": 0,
        "unit": ""
      },
      {
  ...
  "technic-14486": {
    ...
    "stats": {
      "attackPct": 3,
      "extraACPct": 10,
      "extraMaxWeightPct": 20
    },
    "skillEffects": [],
    "customEffects": [
      {
        "name": "専用技ドラゴン フォームと派生技3種を使用可能",
        "value": 0,
        "unit": ""
      },
      {
  ...
  "technic-14483": {
    ...
    "customEffects": [
      {
        "name": "自然回復増加・ST消費軽減・火属性被効果軽減（数値未確認）",
        "value": 0,
        "unit": ""
      },
      {
  ...
  "technic-14482": {
    ...
    "customEffects": [
      {
        "name": "致死ダメージを耐えるとBuff消失・一定時間で再点灯（付替後約8秒は不点灯）",
        "value": 0,
        "unit": ""
      },
      {
